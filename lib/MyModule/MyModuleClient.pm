package MyModule::MyModuleClient;
...
sub new
{
    ...
    my $self = {
	client => MyModule::MyModuleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a MyModule.FilterContigsParams
$return is a MyModule.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a MyModule.workspace_name
	contigset_id has a value which is a MyModule.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a MyModule.ws_contigset_id
...
# Authentication: required
	...
	method => "MyModule.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MyModule::MyModuleClient\n";
    }
    if ($sMajor == 0) {
        warn "MyModule::MyModuleClient version is $svr_version. API subject to change.\n";
    }
}
...
package MyModule::MyModuleClient::RpcClient;
