...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'MyModule'):
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['MyModule.filter_contigs_async'] = ['MyModule', 'filter_contigs']
async_check_methods['MyModule.filter_contigs_check'] = ['MyModule', 'filter_contigs']
sync_methods['MyModule.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'MyModule'
        ...
        self.rpc_service.add(impl_MyModule.filter_contigs,
                             name='MyModule.filter_contigs',
                             types=[dict])
        self.method_authentication['MyModule.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "MyModule but no authentication header was passed"
                            raise err
